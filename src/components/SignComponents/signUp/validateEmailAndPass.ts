...
const arrayOfValidate: { [key: string]: Function } = {
    ...
    login: ({value, valid}: { value: string, valid: string }): boolean => {
        ...
        const a = parsedArr.find((obj: any) => obj.login === value && obj.password === valid);
        console.log(value,valid,a,parsedArr);
        return !!a;
    }
}

export const validateEmailAndPass: Function = ({fields, accParams}: ValidateRegData): boolean => {
    let isValid: boolean = true;
    fields.forEach((obj, index, array) => {
        const a: string = Object.keys(accParams).filter((key) => key === obj.id)[0];
        if (obj.id === a) {
            obj.validations.filter(obj => arrayOfValidate[obj.type]({
                value: accParams[a],
                valid: obj.validValue === 'password' ? accParams["pass"] : obj.validValue
            }) === false).length && (isValid = false);
        }
    });
    return isValid;
}