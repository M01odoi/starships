...
const FormBuilder: React.FC<Props> = ({
    ...
    const isValid: Function = ({
                                   obj,
                                   validationErrors
                               }: { obj: IFormConfig, validationErrors: IValidationErrors[] }): IValidationErrors[] => {
        return validationErrors.filter((validateObj) => validateObj.field === obj.id)
    }
    useEffect(() => {
        ...
        <form>
            {fields.map((obj,index) => (
                <>
                    ...
                    name={obj.name}
                    className={isValid({obj, validationErrors}).length
                        ? "dontCorrectValidate" : ""}
                ...
                />
                    {!!isValid({obj, validationErrors}).length && isValid({
                        obj,
                        validationErrors
                    }).map((error: IValidationErrors,index:number) => <span key={index}>{error.message}</span>)}
                </>
            ))}
            <button
